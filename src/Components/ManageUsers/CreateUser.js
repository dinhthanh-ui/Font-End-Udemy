...
const CreateUser = () =>
{
	...
	const handCreateAccount = async (userData) =>
	{
		...
		*/
		...
		let checkPassword = /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z\d]{8,16}$/;
		...
		let message = await createUsers(fullName, email, password, phone, address, gender, userName)
		...
		{
			if (message.errorMessage === "Email hoặc tên người dùng và điện thoại đã tồn tại, Vui lòng nhập một địa chỉ email mới !!!!" || message.errorData === "Email")
			{
				toast.error(message.errorMessage)
				setCheckInput({ ...checkValid, isValidEmail: false })
			}
			else if (message.errorMessage === "Email hoặc tên người dùng và điện thoại đã tồn tại, Vui lòng nhập một địa chỉ email mới !!!!" || message.errorData === "phone")
			{
				toast.error(message.errorMessage)
				setCheckInput({ ...checkValid, isValidPhone: false })
			}
					...
					<div className="col">
						<div className="card card-registration">
							<div className="row g-0">
								...
								<form className="col-xl-6 my-5" onSubmit={handleSubmit(handCreateAccount)} >
									<div className="card-body p-md-3 text-black">
										...
										<div className="d-md-flex justify-content-start align-items-center mb-2 py-2">
											...
											<div className="form-check form-check-inline mb-0">
												...
												<label className="form-check-label">Giới tích thứ 3</label>
											</div>
										</div>
										...
										<div className="d-flex justify-content-end pt-3">
											<button type="button" className="btn btn-light btn-lg ms-3 text-uppercase" onClick={() => handUsers()}>Back To User</button>
